"use client";

import { useUser, useClerk } from "@clerk/nextjs";
import { useState, useEffect, useRef } from "react";
import Link from "next/link";
import { motion, AnimatePresence } from 'framer-motion';

type UserProfileModalProps = {
  isOpen: boolean;
  onClose: () => void;
};

export function UserProfileModal({ isOpen, onClose }: UserProfileModalProps) {
  const { user } = useUser();
  const { signOut } = useClerk();
  const [initials, setInitials] = useState("");
  const modalRef = useRef<HTMLDivElement>(null);
  
  useEffect(() => {
    if (user?.firstName && user?.lastName) {
      setInitials(`${user.firstName.charAt(0)}${user.lastName.charAt(0)}`);
    }
  }, [user?.firstName, user?.lastName]);

  // Handle click outside
  useEffect(() => {
    function handleClickOutside(event: MouseEvent): void {
      if (modalRef.current && !modalRef.current.contains(event.target as Node)) {
        onClose();
      }
    }
    
    document.addEventListener('mousedown', handleClickOutside);
    return () => {
      document.removeEventListener('mousedown', handleClickOutside);
    };
  }, [onClose]);

  if (!isOpen || !user) return null;

  return (
    <AnimatePresence>
      <div className="fixed top-16 right-4 z-50 flex items-start justify-end">
        <motion.div 
          ref={modalRef}
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          exit={{ opacity: 0, y: -20 }}
          transition={{ duration: 0.2 }}
          className="bg-white rounded-lg shadow-2xl border border-gray-200 w-full max-w-md overflow-hidden"
        >
          {/* Header with close button */}
          <div className="flex justify-between items-center p-4 border-b">
            <h2 className="text-xl font-semibold">Профиль пользователя</h2>
            <button 
              onClick={onClose}
              className="text-gray-500 hover:text-gray-700"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          </div>
          
          {/* User info */}
          <div className="p-6">
            <div className="flex flex-col items-center mb-6">
              {user.imageUrl ? (
                <img 
                  src={user.imageUrl} 
                  alt={user.fullName || "Пользователь"} 
                  className="w-24 h-24 rounded-full object-cover border-2 border-primary-600"
                />
              ) : (
                <div className="w-24 h-24 rounded-full bg-primary-600 flex items-center justify-center text-white text-2xl font-bold">
                  {initials}
                </div>
              )}
              <h3 className="mt-4 text-xl font-semibold">{user.fullName}</h3>
              <p className="text-gray-600">{user.primaryEmailAddress?.emailAddress}</p>
            </div>
            
            <div className="space-y-4">
              <div className="border-t pt-4">
                <h4 className="text-sm font-medium text-gray-500 mb-2">Аккаунты</h4>
                <div className="space-y-2">
                  {user.externalAccounts.map(account => (
                    <div key={account.id} className="flex items-center">
                      <div className="w-8 h-8 flex-shrink-0 mr-3">
                        {account.provider === "google" && (
                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
                            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                          </svg>
                        )}
                        {account.provider === "github" && (
                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
                            <path fill="#24292e" d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
                          </svg>
                        )}
                      </div>
                      <span className="text-sm">{account.provider.charAt(0).toUpperCase() + account.provider.slice(1)}</span>
                    </div>
                  ))}
                </div>
              </div>
              
              <div className="border-t pt-4">
                <h4 className="text-sm font-medium text-gray-500 mb-2">Информация</h4>
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div className="text-gray-500">ID пользователя:</div>
                  <div className="font-mono text-xs truncate">{user.id}</div>
                  <div className="text-gray-500">Дата регистрации:</div>
                  <div>{user.createdAt ? new Date(user.createdAt).toLocaleDateString('ru-RU') : '-'}</div>
                  <div className="text-gray-500">Последний вход:</div>
                  <div>{user.lastSignInAt ? new Date(user.lastSignInAt).toLocaleDateString('ru-RU') : '-'}</div>
                </div>
              </div>
              
              {/* Sign out button */}
              <div className="border-t pt-4">
                <button 
                  onClick={() => signOut()}
                  className="w-full py-2 px-4 bg-red-600 hover:bg-red-700 text-white font-medium rounded-md transition-colors"
                >
                  Выйти
                </button>
              </div>
            </div>
          </div>
        </motion.div>
      </div>
    </AnimatePresence>
  );
} 